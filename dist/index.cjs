...
__export(index_exports, {
  AccountMalt: () => AccountMalt,
  AgentMalt: () => AgentMalt,
  AnalyticMalt: () => AnalyticMalt,
  AnalyticsMalt: () => AnalyticsMalt,
  BidMalt: () => BidMalt,
  BlockMalt: () => BlockMalt,
  BookMalt: () => BookMalt,
  ChartMalt: () => ChartMalt,
  ClaimableMalt: () => ClaimableMalt,
  CoinMalt: () => coin_default,
  ErrorsMalt: () => ErrorsMalt,
  ExtensionsMalt: () => ExtensionsMalt,
  FaultMalt: () => FaultMalt,
  FiltersMalt: () => FiltersMalt,
  JsonMalt: () => JsonMalt,
  KeysMalt: () => KeysMalt,
  LayoutMalt: () => LayoutMalt,
  MailerMalt: () => MailerMalt,
  Malt: () => Malt,
  MessageMalt: () => MessageMalt,
  MimesMalt: () => MimesMalt,
  ParamsMalt: () => ParamsMalt,
  QueryMalt: () => QueryMalt,
  RouteMalt: () => RouteMalt,
  SectionMalt: () => SectionMalt,
  ShareMalt: () => ShareMalt,
  TradeMalt: () => TradeMalt
});
...
var Malt = class _Malt {
  ...
  formatPlusPhone(phone, defaultCountry = "US") {
    const phoneNumber = (0, import_libphonenumber_js.parsePhoneNumberFromString)(phone, defaultCountry);
    if (phoneNumber) {
  ...
  safeDecrypt(m, s) {
    ...
    const decipher = import_crypto.default.createDecipheriv(algorithm, s, iv);
    let decrypted = decipher.update(encryptedData, void 0, "utf-8");
    decrypted += decipher.final("utf-8");
    const computedHmac = import_crypto.default.createHmac("sha256", s).update(encryptedData).digest();
    if (import_crypto.default.timingSafeEqual(hmac, computedHmac)) {
      t = decrypted;
    }
    return t;
  }
  safeEncrypt(plainText, secret) {
    const algorithm = "aes-128-cbc";
    const iv = import_crypto.default.randomBytes(16);
    const cipher = import_crypto.default.createCipheriv(algorithm, secret, iv);
    let encrypted = cipher.update(plainText, "utf8", "base64");
    encrypted += cipher.final("base64");
    const hmac = import_crypto.default.createHmac("sha256", secret).update(Buffer.from(encrypted, "base64")).digest();
